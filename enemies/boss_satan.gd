...
# Satan — red laser, drops Megacluster Cannon on death
...
func _build_visual():
	...
	for i in 8:
		...
		ray.color = Color(0.9, 0.1, 0.0, 0.6)
	...
	# 2 horns
	var horn1 = Polygon2D.new()
	horn1.color = Color(0.3, 0.05, 0.0)
	horn1.polygon = PackedVector2Array([Vector2(-8, -18), Vector2(-5, -35), Vector2(-2, -18)])
	body.add_child(horn1)

	var horn2 = Polygon2D.new()
	horn2.color = Color(0.3, 0.05, 0.0)
	horn2.polygon = PackedVector2Array([Vector2(8, -18), Vector2(5, -35), Vector2(2, -18)])
	body.add_child(horn2)

	# Central evil eye
	var eye = Polygon2D.new()
	eye.color = Color(1, 0.2, 0.0)
	...
	laser_line.default_color = Color(1, 0.1, 0.05, 0.9)
...
func die():
	...
	# Drop Megacluster Cannon!
	var player = get_tree().get_first_node_in_group("player")
	if player:
		player.add_weapon("megacluster_cannon")
	# Floating text
	var label = Label.new()
	label.text = "MEGACLUSTER CANNON!"
	label.global_position = global_position + Vector2(-80, -30)
	label.add_theme_font_size_override("font_size", 24)
	label.add_theme_color_override("font_color", Color(1, 0.3, 0.0))
	label.z_index = 100
